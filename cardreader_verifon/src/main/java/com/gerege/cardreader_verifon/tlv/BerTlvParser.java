...
public class BerTlvParser {
    ...
    private ParseResult parseWithResult(int aLevel, byte[] aBuf, int aOffset, int aLen) {
        ...
        if (log.isDebugEnabled()) {
            log.debug("{}parseWithResult(level={}, offset={}, len={}, buf={})", levelPadding, aLevel, aOffset, aLen, Helper.toFormattedHexString(aBuf, aOffset, aLen));
        }
        ...
        if (log.isDebugEnabled()) {
            log.debug("{}tag = {}, tagBytesCount={}, tagBuf={}", levelPadding, tag, tagBytesCount, Helper.toFormattedHexString(aBuf, aOffset, tagBytesCount));
        }
        ...
        if (log.isDebugEnabled()) {
            log.debug("{}lenBytesCount = {}, len = {}, lenBuf = {}"
                    , levelPadding, lengthBytesCount, valueLength, Helper.toFormattedHexString(aBuf, aOffset + tagBytesCount, lengthBytesCount));
        }
        ...
        if (tag.isConstructed()) {
            ...
            int resultOffset = aOffset + tagBytesCount + lengthBytesCount + valueLength;
            if (log.isDebugEnabled()) {
                log.debug("{}value = {}", levelPadding, Helper.toFormattedHexString(value));
    ...
    private BerTag createTag(String aLevelPadding, byte[] aBuf, int aOffset, int aLength) {
        if (log.isDebugEnabled()) {
            log.debug("{}Creating tag {}...", aLevelPadding, Helper.toFormattedHexString(aBuf, aOffset, aLength));
        }
